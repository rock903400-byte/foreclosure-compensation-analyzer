...
for item in json_data:
    if str(item.get("saleno")) not in ["1", "2", "3", "4", "6"]:
        continue
    if str(item.get("rrange", "")).strip() not in ["", "全部", "1分之1"]:
        continue

    # Normalize saledate
    saledate_raw = str(item.get("saledate", "0")).strip()
    saledate = saledate_raw.zfill(8)  # pad to 8 digits

    if saledate < "01150428":
        continue
    if not any(k in str(item.get("ctmd", "")) for k in keywords):
        continue
...
if extra:
    ...
    for e in extra[:5]:
        item = e["item"]
        print(
            f"  Key: {e['key']} | saleno: {item.get('saleno')} | saledate: {item.get('saledate')} | ctmd: {item.get('ctmd')}"
        )
