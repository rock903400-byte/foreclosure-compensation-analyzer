...
class AuctionSystemTester:
    ...
    def _generate_mock_data(self):
        ...
        for i in range(300):  # 生成300個模擬拍賣項目
            item = {
                ...
                "pdf_url": (
                    f"https://example.com/pdf/{i+1}.pdf"
    ...
    def _simulate_single_user(self, user, results_lock):
        ...
        if self.simulate_user_login(user):
            ...
            with results_lock:
                print(
                    f"   User {user.user_id} ({user.role}) - accessed {accessible_count} items"
                )
                if ui_issues:
    ...
    def run_100_user_test(self):
        ...
        for i in range(1, 101):
            ...
            role = random.choices(
                list(role_weights.keys()), weights=list(role_weights.values())
            )[0]
        ...
        for user in self.users:
            t = threading.Thread(
                target=self._simulate_single_user, args=(user, results_lock)
            )
            threads.append(t)
            t.start()
    ...
    def _generate_report(self, total_time):
        ...
        print("\nLogin Statistics:")
        print(
            f"   Success: {self.test_results['login_success']} ({self.test_results['login_success']/100*100:.1f}%)"
        )
        print(
            f"   Failed: {self.test_results['login_failed']} ({self.test_results['login_failed']/100*100:.1f}%)"
        )

        # UI響應時間
        if self.test_results["ui_response_time"]:
            avg_response = sum(self.test_results["ui_response_time"]) / len(
                self.test_results["ui_response_time"]
            )
            ...
            if avg_response > 2.0:
                print("   Warning: Average response time too slow (>2 seconds)")
        ...
        if self.test_results["data_access_violations"]:
            ...
            for violation in self.test_results["data_access_violations"][:3]:
                print(
                    f"   - {violation['user']}: Role {violation['role']} attempted to access {violation['attempted_access']} items (limit: {violation['limit']})"
                )
        else:
        ...
        if self.test_results["login_success"] / 100 < 0.9:
            print("   - Improve login mechanism to increase success rate")
        if (
            self.test_results["ui_response_time"]
            and max(self.test_results["ui_response_time"]) > 3.0
        ):
            print("   - Optimize backend response speed to reduce delays")
        if self.test_results["data_access_violations"]:
            print("   - Strengthen access control mechanism to prevent violations")
        if self.test_results["ui_issues"]:
            print("   - Fix UI issues to improve user experience")
